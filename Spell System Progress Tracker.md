...
| `SetBusy` | Partial | Decodes `DataBits00=1` as set busy and `DataBits00=0` as clear busy, tracks effect-id busy latches on `IWorldEntity` targets, broadcasts/replays busy state through `ServerUnitInUse`, rejects direct activate/interact requests on busy targets, and removes duration-backed busy state; CSI/deferred interaction parity and context-id semantics remain. |
...
| `Proxy` / non-random variants | Conservative | Chained casts and target forwarding exist, and plain `Proxy` now also runs on activation-object world targets by casting the child spell from the original caster while forwarding the object target as the child primary target; random-exclusive, channel ownership, and shape forwarding remain. |
...
| `SpellForceRemove` variants | Conservative | Cleans tracked properties, CC, stealth, aggro immunity, absorption, and other tracked local states. On activation-object world targets, the conservative shell now removes matching busy latches only, because busy is the only tracked object spell state today. Remove type `2` uses concrete `Spell4`; remove type `3` uses `Spell4Base`; type `1` and count/stack fields remain. |
...
| `RavelSignal` | Structural diagnostics | Decodes `DataBits00` as signal mode, `DataBits01` as the likely signal id, preserves remaining payload fields, shows semantics in `/spell inspect4`, and emits `ravel-signal` diagnostics for both unit and activation-object world targets. No receiver/script mutation is implemented yet. |
| `DespawnUnit` | Conservative | Delayed non-player map removal now works for both unit and activation-object world targets; non-zero payload modes remain. |
...
| `Activate` | Partial | Quest activation objective updates, the current world-target activation-object slice (`Proxy`, `Activate`, `SetBusy`, conservative busy-only `SpellForceRemove`, `DespawnUnit`, diagnostics-only `RavelSignal`, and `Fluff`), and `ClientActivateUnitCast` spell resolution now work for object interactions; CSI/script/visibility payload behavior remains. |
...
| ---: | --- | --- | --- |
| 1 | Target mechanics and valid target masks | Affects every family and prevents wrong target application. | **`SpellTarget_ResolveTargetEntity` (`14055bdc0`) fully decoded from 350-function Ghidra export. TargetType switch confirmed: types 0/2/6 → caster (self); types 1/3/5/7/8 → current target; type 4/default → no entity. TargetType 6 now returns `Owner.Guid` (item-activation, confirmed self). TargetTypes 5 and 8 now use current-target path (were returning 0u). Auto-target bitmask 0x12a (types 1/3/5/8, ~5 yard seek). Remaining: `DAT_140c65b70 + 0x788` service tree (type-7 per-spell overrides), non-dead valid-target masks, world-target family routing beyond `0x02`.** |
