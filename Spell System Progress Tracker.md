...
| ---: | --- | --- | --- |
| 1 | Target mechanics and valid target masks | Affects every family and prevents wrong target application. | **TargetTypes 0 and 7 decoded from `Game_Spell_IsSelfSpellDelegate` (`1403b4ec0`) bitmask `0x85` (bits 0/2/7 = self-spell). TargetType 0 now returns `Owner.Guid` (like SelfAoe); TargetType 7 now uses current target (auto-attack conservative path). TargetType 6 (mechanic 17 only, item-activation) remains `return 0u` pending live item2 witness. Remaining work: `DAT_140c65b70 + 0x788` service tree (type-7 per-spell overrides), non-dead valid-target masks, world-target family routing beyond `0x02`.** |
