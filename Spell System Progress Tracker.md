...
| `SapVital` | Conservative | Decodes `DataBits00` as `Vital`, treats clean `DataBits01/02` payloads as percent-of-max restore/drain amounts by mode, and emits `CombatLogVitalModifier`; parameter-driven, secondary-payload, unsupported vital, and out-of-range rows remain diagnostic-only. |
...
| `HealShields` | Conservative | Formula path restores shield capacity with overheal tracking and `CombatLogHeal`; packet parity and HoT cadence need validation. |
...
| `UnitStateSet` | Structural | Decodes and tracks raw unit state ids by effect id, emits diagnostics, removes duration-backed states, and participates in force-remove/dispel cleanup; individual state-id behavior such as block/invulnerability remains evidence-only. |
| `SetBusy` | Structural | Decodes `DataBits00=1` as set busy and `DataBits00=0` as clear busy, tracks effect-id busy latches, supports delayed same-spell/context clears, and removes duration-backed busy state; UI/object interaction parity remains. |
...
| `UnitPropertyModifier` | Partial | Adds/removes timed modifiers; stack groups, persistence refresh, and packet parity remain. |
| `PersonalDmgHealMod` | Conservative | Maps common personal damage/heal modifier codes to existing outgoing damage, incoming damage, and healing multiplier properties; duration cleanup and damage-taken application are wired, while uncommon codes and auxiliary fields remain diagnostic-only. |
| `CCStateSet` | Partial | Set/remove packets, logs, active mask, and timed removal exist; DR, breakout, interrupt armor, tether, and movement coupling remain. |
| `CCStateBreak` | Partial | Removes tracked CC states and emits break logs; small-payload semantics remain. |
...
| `ItemVisualSwap` | Conservative | Applies immediate client visual slot/display swaps through `AddVisual`; duration-backed rows are traced until previous-visual restore tracking is implemented. |
...
| `Proc` | Structural + probes | Decodes trigger event, trigger `Spell4`, float-bitcast chance, target data, cooldown/sentinel, and remaining raw fields; tracks proc state by effect id, supports duration cleanup and force-remove/dispel cleanup, emits `proc` diagnostics, and now emits diagnostic-only `proc-probe` rows for cast, combat-entry, damage, heal, shield-damage, and kill contexts. Event dispatch, target-data routing, internal cooldown consumption, and recursion rules remain blocked on sniff/client confirmation. |
...
| 2 | `Proc` | 3,498 rows; likely central to buffs/items/runes. | Use `proc-probe` traces to prove trigger-event enum, `targetData` routing, cooldown/recursion rules, and safe event hooks from sniff/client evidence before enabling trigger casts. |
...
| 9 | `UnitStateSet` validation | Broad state latch family is now tracked structurally, but state ids need behavior naming. | Validate state `1` block rows, state `6` invulnerability/frozen rows, state `7/8/9/16/17/18` all-spell immunity rows, and state `22/23` barrier/burrow rows before adding combat gates. |
| 10 | `SetBusy` validation | Activation/object busy state is now structurally wired from paired set/clear rows. | Validate instant set plus delayed clear rows, duration-backed rows, non-zero context ids, and whether busy should suppress activation/CSI/object use. |
...
| 18 | Stack groups and persistence | Needed for buff/debuff correctness. | Add tracking around `Spell4StackGroup` and persistence prereqs. |
