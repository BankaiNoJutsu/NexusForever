...
{
    public partial class CombatAI : IUnitScript, IOwnedScript<ICreatureEntity>
    {
        ...
        public virtual void OnLoad(ICreatureEntity owner)
        {
            ...
            NormalizeStarterTutorialCombatFaction();
        ...
        private void AggroEntity(IGridEntity source, bool requireAggroRange)
        {
            ...
            if (requireAggroRange && !CanIdleAggroTarget(unit))
            {
                if (ShouldLogStarterTutorialCombat(unit))
                    log.LogTrace("Starter tutorial combat AI aggro skipped for creature {CreatureId} guid {Guid}: source unit {SourceGuid} is not eligible for idle aggro.", entity.CreatureId, entity.Guid, unit.Guid);
        ...
        {
            if (!TryGetStarterTutorialCombatFaction(entity.CreatureId, out Faction faction))
                return;

            if (entity.Faction1 == faction && entity.Faction2 == faction)
                return;
            ...
            if (profile.TraceCombat)
                log.LogDebug("Starter tutorial combat AI normalized creature {CreatureId} guid {Guid} to faction {Faction}.", entity.CreatureId, entity.Guid, faction);
        }

        private static bool TryGetStarterTutorialCombatFaction(uint creatureId, out Faction faction)
        {
            faction = creatureId switch
            {
                73464u or 73492u or 73567u or 73494u => TutorialDominionCombatFaction,
                73465u or 73473u or 73566u or 74862u => TutorialExileCombatFaction,
                _ => Faction.None
            };
