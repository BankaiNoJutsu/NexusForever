...
public class MatchingGameMap126Tests
{
    [Fact]
    public void PrimeUltimateProtogamesMap126_ExactMetadataRequiresRoleSelection()
    {
        MatchingMap matchingMap = CreateMatchingMap126();
        ...
        Assert.Equal(126u, matchingMap.Id);
        Assert.Equal(2980u, matchingMap.GameMapEntry.WorldId);
        Assert.Equal(81u, matchingMap.GameMapEntry.MatchingGameTypeId);
        Assert.Equal(80u, matchingMap.GameMapEntry.RecommendedItemLevel);
        Assert.Equal(316u, matchingMap.GameMapEntry.AchievementCategoryId);
        Assert.Equal(MatchingMatchType.PrimeLevelDungeon, matchingMap.GameTypeEntry.MatchTypeEnum);
        ...
        Assert.False(matchingDataManager.RequiresRoleSelection(MatchingMatchType.PrimeLevelExpedition));
    }

    [Fact]
    public void PrimeUltimateProtogamesMap126_QueueValidatorAcceptsExactLevelAndRole()
    {
        MatchingQueueValidator validator = CreateValidator(
            level: 50u,
            matchType: MatchingMatchType.PrimeLevelDungeon,
            hasMapEntrance: true,
            roles: Role.DPS,
    ...
    [Theory]
    [InlineData(49u, MatchingMatchType.PrimeLevelDungeon, true, Role.DPS, MatchingQueueResult.Level)]
    [InlineData(50u, MatchingMatchType.ScaledPrimeLevelDungeon, true, Role.DPS, MatchingQueueResult.TypeMismatch)]
    [InlineData(50u, MatchingMatchType.PrimeLevelDungeon, false, Role.DPS, MatchingQueueResult.UnableToQueue)]
    [InlineData(50u, MatchingMatchType.PrimeLevelDungeon, true, Role.None, MatchingQueueResult.GroupSize)]
    public void PrimeUltimateProtogamesMap126_QueueValidatorRejectsInvalidBoundary(
        uint level,
        ...
        out IMatchingQueueProposal proposal)
    {
        ...
        proposalProxy.SetMethodReturn(nameof(IMatchingQueueProposal.GetMatchingMaps), new IMatchingMap[] { CreateMatchingMap126() });
    ...
    private static MatchingMap CreateMatchingMap126()
    {
        return new MatchingMap
        {
            GameMapEntry = new MatchingGameMapEntry
            {
                ...
                LocalizedTextIdName       = 764203u,
                LocalizedTextIdDescription = 764204u,
                MatchingGameTypeId        = 81u,
                WorldId                   = 2980u,
                RecommendedItemLevel      = 80u,
            ...
            {
                Id                        = 81u,
                LocalizedTextIdName       = 763573u,
                LocalizedTextIdDescription = 763574u,
                MatchTypeEnum             = MatchingMatchType.PrimeLevelDungeon,
                MatchingGameTypeEnumFlags = 163u,
                TeamSize                  = 5u,
                MinLevel                  = 50u,
                MaxLevel                  = 50u
            }
