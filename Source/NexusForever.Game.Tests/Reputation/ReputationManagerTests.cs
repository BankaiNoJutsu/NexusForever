...
public class ReputationManagerTests
{
    ...
    public void UpdateReputation_NewHighStandingAwardsEveryReachedFactionLevel()
    {
        ...
        LegacyServiceProvider.Provider = BuildFactionProvider(Faction.Dominion, factionNode);

        try
        {
            var manager = new ReputationManager(player, new CharacterModel { Id = 42ul });
    ...
    public void UpdateReputation_LosingStandingDoesNotAwardProgressionAchievements()
    {
        ...
        try
        {
            ...
            var manager = new ReputationManager(player, model);
    ...
    private static IServiceProvider BuildFactionProvider(Faction factionId, IFactionNode node)
    {
        var factionManager = new FactionManager();
        typeof(FactionManager)
            .GetField("nodes", BindingFlags.Instance | BindingFlags.NonPublic)!
            .SetValue(factionManager, ImmutableDictionary<Faction, IFactionNode>.Empty.Add(factionId, node));
