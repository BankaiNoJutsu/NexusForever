...
Last updated: 2026-05-23 (replacement queue hardening, group stat mapping, crafting/map-tracked review)
...
| Tests | 928 passed, 0 failed, 0 skipped |
...
### F-010 - Group / Raid / Matching - PARTIAL (replacement live smoke remains)
...
- `ClientMatchingMatchInitiateLookingForReplacementsHandler` - validates match
  state and initiates a replacement queue via `IMatchingManager.TryStartLookingForReplacements`
- `ClientMatchingStopLookingForReplacementsHandler` - stops replacement queue
- `MatchingQueueGroup.SetInProgress()` + matching queue integration
- Accepted replacements merge into the existing in-progress match; stale replacement groups close instead of creating a fresh match
