...
Last updated: 2026-05-23 (server opcode placeholder elimination, account-gift pending groups, raid queue status diagnostics)
...
| Tests | 897 passed, 0 failed, 0 skipped |
| Handlers surveyed | 200+ |
| Stubs found | 0 |
| **Ghidra decomp evidence (2026-05-23 pass 2)** | Server opcode coverage structurally closed at 699/699; entity aux reader shapes remain mapped; registration-table helper scripts reviewed |
...
### F-010 - Group / Raid / Matching - PARTIAL (diagnostic gaps remain)
**Fixed this pass:** `ClientRaidInfoRequestHandler` now follows the raid-lock
response with a conservative zeroed `ServerRaidQueueStatus`. Looking-for-
replacements handlers validate match state and membership, then log diagnostic
intent only.
...
looking-for-replacements full backfill remains blocked until queue proposals can
attach to an existing in-progress match instead of creating a fresh match.
...
| # | Feature | Gap | Blocker |
|---|---------|-----|---------|
| 1 | F-010 | ServerRaidQueueStatus (0x0718) non-zero semantics/timing | Decomp raid queue consumer or retail capture |
| 2 | F-010 | Looking-for-replacements (matching queue) | Queue-to-existing-match attachment model |
...
| 14 | F-025 | 62 shape-mapped aux/spell packets | Consumer-handler decomp, server emit sites, or retail captures |
