...
Last updated: 2026-05-23 (quest objective coverage, looking-for-replacements diagnostics, decompile helper review)
...
| Tests | 889 passed, 0 failed, 0 skipped |
| Handlers surveyed | 200+ |
| Stubs found | 0 |
| **Ghidra decomp evidence (2026-05-23 pass 2)** | Entity aux reader shapes remain modeled; registration-table helper scripts reviewed; looking-for-replacements diagnostics added |
| **Code Review Backlog review (2026-05-23)** | Runtime fixes applied; speculative crafting discovery remains blocked |
...
| 15 | O(n²) commodity matching | Added XML doc documenting O(n²) with note about price-indexed refactor. |
...
Looking-for-replacements handlers (2) - now validate match state + membership; full matching queue integration pending.
...
labels exist and field-level decomp remains tracked against C# models. Consumer
dispatch semantics remain blocked (indirect calls through the function pointer
registration table).
...
| 2 | F-010 | Looking-for-replacements (matching queue) | Full matching queue integration for InProgress groups |
...
|---------|----------|---------|-------------|
| 0x140095c20 | Server0x025F_ReadPayload | 0x025F (44 bytes) | ushort + uint + 3xuint32 + uint + 3xuint32 + uint + uint |
| 0x140095ce0 | (opcode 0x0260/0x0261 consumer) | 0x0260/0x0261 | counted rows of 0x025F/0x0263 |
| 0x140095a80 | (opcode 0x0261 consumer) | 0x0261 | counted rows of 0x0263 |
| 0x1400959c0 | Server0x0263_ReadPayload | 0x0263 (36 bytes) | uint + uint + 3x17bit + uint + 3xuint32 |
| 0x140095b40 | Server0x0264_ReadPayload | 0x0264 (24 bytes) | uint + ushort + uint + counted uint list |
| 0x140097620 | Server0x08F4_ReadPayload | 0x08F4/0x087F/0x0938 (12 bytes) | uint + 5-bit + uint |
| 0x140097690 | Server0x093D_ReadPayload | 0x093D (16 bytes) | uint + 5-bit + uint + uint |
| 0x140097ee0 | Server0x0939_ReadPayload | 0x0939 (24 bytes) | uint + 14-bit + 18-bit + wide string |
| 0x140097f70 | Server0x093E_ReadPayload | 0x093E (16 bytes) | uint + uint + uint64 |
