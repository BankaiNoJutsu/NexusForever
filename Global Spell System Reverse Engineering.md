...
| `Proxy` plus non-random proxy variants | 17,542 | Casts chained spell id and emits `proxy` diagnostics for `Proxy`, `ProxyLinearAE`, `ProxyChannel`, and `ProxyChannelVariableTime`; forwards the realized proxy target as the child primary target, and plain `Proxy` now also routes activation-object world targets by casting the child spell from the original caster; channel ownership and random-exclusive semantics need work. |
...
| `SpellForceRemove` / `SpellForceRemoveChanneled` | 2,469 | Removes locally tracked spell states by concrete `Spell4` id for type `2` and by `Spell4Base` id for type `3`; activation-object world targets now also clear matching busy latches conservatively because busy is the only tracked object spell state today. Type `1` and stack/count payloads remain open. |
...
| `RavelSignal` | 5,262 | Decodes mode, likely signal id, and raw payload fields; `/spell inspect4` shows the shape and the handler emits `ravel-signal` diagnostics for both unit and activation-object world targets, but no script/receiver mutation is implemented yet. |
...
| `SetBusy` | 571 | Tracks `DataBits00=1` busy rows and clears them through `DataBits00=0` same-spell/context rows or duration cleanup on `WorldEntity` targets, reuses `ServerUnitInUse` for visible busy state, and conservatively blocks direct activate/interact requests; CSI/deferred interaction parity and context-id semantics remain open. |
...
| `DespawnUnit` | 855 | Removes non-player world entities from the map after delay scheduling, including activation-object world targets; non-zero payload modes remain open. |
...
| `CCStateBreak` | 420 | Removes tracked active CC states by decoded state mask, preserves original remove cast identity, and routes player knockdown breakout through the same tracked-removal path; small-payload semantics and broader breakout parity need validation. |
...
| `GiveAbilityPointsToPlayer` | 1 | Grants the single observed bonus ability tier point across action sets, sends the native available/total budget through `ServerAbilityPoints`, and persists the expanded budget before saved loadouts are reconstructed. |
| `TradeSkillProfession` / `PathMissionIncrement` | 5 | Learns the three table-backed hobbies through `Player.LearnTradeskill` and increments the two active Soldier SWAT missions through the shared table-backed path progress/completion pipeline. |
...
Client test spell names give high-confidence labels for several target types: `1` single target, `2` self AOE, `3` target AOE, `4` position AOE, and `5` chain target. The runtime now uses only the structurally safe subset of that mapping: player casts forward the selected target for type `1`, `3`, and `5` through `IWorldEntity` visibility rather than unit-only resolution; type `3` telegraphs anchor on the primary target position when present; and type `4` telegraphs anchor on the supplied spell position or primary-target fallback when available.
...
Fresh `DumpNearbyData` output on the raw `SapVital` string at `140abb710` places it inside a contiguous spell-effect name block beside `ActionBarSet`, `SummonCreature`, `ForceFacing`, `Absorption`, `CCStateBreak`, and `ProxyChannel`. That is strong registration-table evidence for the family even though the focused export still lacks a direct gameplay-code xref for the string.

The runtime now decodes `SapVital`, skips parameter and secondary-payload rows, treats `DataBits02` values up to `100` as percent-of-max and `DataBits01` values up to `1.0` as ratio-of-max, applies mode `1` positive values as restores, applies modes `0`/`2` and negative mode-`1` values as drains, emits `CombatLogVitalModifier`, and traces all applied/skipped rows through `SpellDiagnostics sap-vital`. Health-vital drains now preserve the spell source through the shared vital helper and only reuse the effect-level `damageType` when it is an explicit physical, tech, magic, fall, or suffocate value; zero or unknown rows still fall back to physical. Open behavior includes the exact mode table, high-scalar/flat payload handling, parameter formulas, unsupported vital aliases, and whether damage-like rows should also emit normal damage/result data alongside the vital-modifier event.
...
The runtime now decodes `SetBusy`, tracks busy state by effect id, clears matching same-spell/context busy rows when a `DataBits00=0` effect executes, removes duration-backed busy rows through the spell lifetime queue, includes busy in force-remove/dispel cleanup, reuses `ServerUnitInUse` for busy-state transitions and visibility replay, rejects direct `ClientActivateUnit`, `ClientActivateUnitCast`, and `ClientEntityInteract` requests on busy targets with `GenericError.TargetBusy` plus `OnActivateFail`, and emits `SpellDiagnostics set-busy`. Open behavior includes exact client/object-state packet parity beyond that conservative shell, whether `IsBusy` should also block CSI/deferred/path/object or AI use in the local interaction layer, the meaning of non-zero `DataBits01` context ids, and whether any rows clear busy state across a broader context than the same spell.

Follow-up activation-object queries tighten the target surface further. Local joins against `Creature2.Spell4IdActivate00..03` show concrete target-flag `2` `SetBusy` rows on real doors, gates, and trigger objects such as `35174`, `41576`, `47634`, and `76797`; the same object-target activation slice also carries `DespawnUnit` on `71338` and `76797`, plus `RavelSignal` on `76797`. That is why the runtime shell now stores busy state on `WorldEntity` rather than only `UnitEntity`, and why non-player world-target execution is widened only for those proven activation families.

### Path XP, Level-Scaled Rewards, And Point Unlocks
...
`GrantLevelScaledPrestige` has twelve rows, all using mode `1`; ten share their `Spell4` with `GrantLevelScaledXP`. The two families use the same cap/mode layout but independent percentage values. That row-level pairing proves the payload shape, but it does not prove that prestige uses the current-level XP span, a particular rounding rule, or the elder/max-level conversion. Because the client handler dispatch does not expose a dedicated prestige formula, the decoded family remains diagnostic-only.
...
`GiveAbilityPointsToPlayer` likewise has one row: effect `169485` on `Spell4Id=67478`, base `44831` (`Class - Ability Tier Point Unlock`), with caster targeting, `DataBits00=1`, and an otherwise zero payload. A focused native pass closed its packet owner: the Lua method table maps `GetAbilityPoints` to `1406feb50`, which reads pending budget `+0x6ddc` with committed `+0x6dd8` fallback, and `GetTotalAbilityPoints` to `1406feb90`, which reads `+0x6de0`. `SpellBook_UpdateTierPointBudget` (`1403b95c0`) writes available and total from the existing two-uint budget packet, then adjusts pending points by the available delta and clamps to total. This proves `ServerAbilityPoints` is the required runtime lane; the historically ambiguous opcode `0x016E` is not needed for the grant.

The runtime now decodes `PathXpModify`, resolves the player owner, and routes mode `0` through `PathManager.AddXp` while mode `1` converts level grants through the `PathLevel` table and existing path reward pipeline. `GrantLevelScaledXP` uses `ceil((nextLevelXp - currentLevelXp) * percent / 100)` and honors the encoded max-level cap. `GrantLevelScaledPrestige` preserves its independently encoded percent/cap/mode values in `/spell inspect4` and trace diagnostics, but does not mutate currency until direct evidence proves the prestige curve, rounding, and elder/max-level conversion. `GiveAugmentPowerToPlayer` increments the runtime AMP bonus-power track up to the encoded ten-point bonus cap, updates all action sets' available AMP power, and sends `ServerAmpPowerUpdate`. `GiveAbilityPointsToPlayer` accepts only the one observed point with zero tail fields, updates all four action sets, caps the bonus at one, sends the available/total pair through `ServerAbilityPoints`, and persists `character.bonusAbilityTierPoints` so a spent 43-point loadout reloads against the expanded ceiling. Open work includes the prestige calculation, the PvP secondary fields on `PathXpModify`, AMP bonus-power persistence, and live-client ordering validation for the ability-point unlock.

### TradeSkillProfession And PathMissionIncrement

`TradeSkillProfession` has exactly three rows in build 16042. `Spell4=32135` carries `DataBits00=19` for Fishing, `33501` carries `20` for Farming, and `34699` carries `2` for Cooking. Those values match the current `TradeskillType` ids, every remaining payload field is zero, and quest rewards already route the same ids through persistent `Player.LearnTradeskill`. The spell handler therefore resolves the player owner and calls `LearnTradeskill(id, 0)`, preserving the existing profession while activating the hobby. Invalid ids and non-zero tail fields are diagnostic-only.

`PathMissionIncrement` has exactly two rows: `Spell4=42373` carries mission `2424` and amount `1`, while `47614` carries mission `3085` and amount `1`. Both `PathMission` rows are Soldier mission type `7`; their object ids resolve to `PathSoldierSWAT 93` with count `5` and `125` with count `15`. The runtime consequently accepts this effect only for an already-active Soldier SWAT mission, adds the decoded amount with saturation, emits the existing `ServerPathMissionUpdate`, and invokes the shared completion/reward pipeline at the SWAT count. Other mission types remain blocked rather than inheriting SWAT semantics.
...
The runtime now decodes `DataBits00` to `CCState`, preserves `DataBits03` as an apply-rules flag payload, decodes `DataBits07` as `CCStateAdditionalData.ID`, emits `CombatLogCCState` with table-backed diminishing-returns ids, sends `ServerEntityCCStateSet`, tracks duration-backed active states on the target, and sends `ServerEntityCCStateRemove` on the duration event. The tracked active-state mask is also used for caster-side `Spell4CCConditions` checks plus a conservative live control shell: clearly named full-control states block casting, `Silence` and `Disarm` are gated by spell school, obvious immobilizes suppress client locomotion, and `Grounded` strips jump flags. This restores a conservative packet/lifetime/control path, but not the full control model: diminishing returns behavior, interrupt armor interactions, stun-direction breakout, forced-move coupling, tether unit/range data, additional-data application, and exact apply-rules result values still need sniff validation.
...
The runtime now removes locally tracked active CC states matching the decoded mask, sends `ServerEntityCCStateRemove` for each removed effect id using the originating cast identity when known, emits `CombatLogCCStateBreak`, routes player `ClientCCStateKnockdownBreak` through the same tracked-removal path for active `Knockdown` states, and suppresses duplicate timed remove packets when the original `CCStateSet` duration event later fires. Retail unknowns remain around small-payload rows, stun breakout/input cadence, immunity windows, interrupt armor interactions, and exact combat-log context.
...
The runtime now decodes force-remove semantics, emits `SpellDiagnostics force-remove`, removes locally tracked spell states by concrete `Spell4` id for type `2` and by `Spell4Base` id for type `3`, and sends matching buff or CC remove packets for affected local state. Activation-object world targets now also use the same remove-type predicate to clear matching busy latches, which is the only world-object spell-owned state currently tracked in runtime. This gives the two strongest cleanup scopes a real shared behavior path without claiming full retail semantics for type `1`, stack/count fields, channeled cleanup timing, or additional packet context.

Local creature activate joins expose a small but useful object-target `SpellForceRemove` slice: 13 parent spells carry target-bit `0x02` `SpellForceRemove` rows, and 7 of those same parents also include `SetBusy` (`28135`, `31568`, `41692`, `42765`, `54657`, `63250`, and `73060`). The remove targets are broader than busy alone, including rows that resolve to `Fluff`, `UnitPropertyModifier`, `ChangeDisplayName`, `FacilityModification`, and `RavelSignal`, so the current world-target runtime deliberately stops at busy-latch cleanup rather than claiming full object-state parity.
...
Placed fixtures include `Spell4Id=63431` (XT-9 Probebot, 29 placements), `72072` (Malfunctioning Scout Drone, 26 placements), `79366` (Firestorm Pyroslinger, 24 placements), `60614` (Metal Maw detonation bombs, 21 placements), and `50212` (Scout Drone, 14 placements). Activation-object joins add world-target despawn witnesses on `71338` (`Scientist Datachron Hacking - CSI Activate`) and `76797` (`Activate and Clear spell effects`).

The runtime now emits `SpellDiagnostics despawn-unit` and calls `RemoveFromMap()` for non-player targets when the scheduled effect executes, including object-target rows that run through the world-target activation slice. Player targets are guarded out until packet/sniff evidence proves a player-facing interpretation. Unknowns remain around non-zero payload modes, object-vs-NPC despawn packet ordering, and whether some player-target rows represent temporary summoned objects rather than the player entity itself.

### Activate

`Activate` has 2,711 rows across 2,708 spells. It is global-content heavy rather than placed `NonPlayer` combat-heavy: the current placed creature join exposes zero `NonPlayer` activate rows, so the best fixtures come directly from global quest/object spells. The stronger activation-object query is the creature activate join: among effect rows with target bit `0x02`, the dominant families are `Activate` (8,183 joined rows), `SetBusy` (1,007), `RavelSignal` (739), `DespawnUnit` (419), and `Fluff` (184). That distribution is why the current world-target routing slice stays narrow and activation-focused.
...
Current server handles `Proxy`, `ProxyLinearAE`, `ProxyChannel`, and `ProxyChannelVariableTime` through the shared chained-cast path: decode `DataBits00`, emit `SpellDiagnostics proxy`, cast the chained `Spell4` with parent/root spell context, forward the realized proxy target as the child spell primary target, and let delayed or duration-bounded periodic rows route through the central spell event queue. Unit-target proxy rows preserve the existing target-as-caster shell, while plain object-target activation proxy rows now cast from the original caster because non-unit targets cannot host `CastSpell()`. Self primary targets now resolve back to the caster, which is important because many child proxy rows put their work on `Target` flags. `ProxyRandomExclusive` is intentionally not registered yet because the family name implies a group-level random choice and casting every row would likely be wrong. Linear AE shape forwarding, channel ownership/timing, random-exclusive selection, and root/parent packet parity still need sniff/client confirmation.

Local creature activate joins prove a small but meaningful object-target `Proxy` slice: 79 rows across 36 concrete parent spells. Representative witnesses include `25513` (`Activate Proxy - Q4070 & Q4072`), `30426` (`Hotwire - CSI Activating w/proxy`), `42816` / `42860` / `44197` (Whitevale adventure CSI/object interactions), `71989` (teleporter control panel scientist activation), and `81591` (`Lockbox Confirmation - Normal Key`). Child spell families are dominated by already familiar activation-side effects: `RavelSignal` (11 joined child rows), `ForcedMove` (10), `Proxy` (7), `CCStateSet` (7), `Fluff` (4), and `ChangeDisplayName` (4), which is why the runtime widens only plain object-target `Proxy` rather than the whole remaining world-target tail.
...
`VectorSlide` is now implemented from its complete 30-row build-16042 family. Mode `0` uses the spell position/caster as a horizontal anchor and applies the signed `DataBits01` float as pull/push velocity; mode `1` applies that signed magnitude along caster facing. Seven duration-bound rows clear velocity through their encoded lifetime. The other 23 rows have zero duration and queue a zero-delay lifetime event, allowing the velocity command to broadcast once before it is cleared on the next spell update instead of remaining latched. The optional `DataBits02=1` flag is preserved in semantics and diagnostics but remains behaviorally unassigned.

The hazard family is also implemented through its existing native packet models. `HazardEnable` creates the full active row and sends `ServerHazardList` before `ServerHazardAction.Enable`, as required by the client apply owner. Server state advances the meter from Hazard.tbl `MeterChangeRate` and freezes that clock while suspended, so later signed deltas do not overwrite a stale initial value. Proven `HazardModify` operation `0..2` / target mode `1` rows apply their signed `DataBits03` meter delta to the `DataBits04` Hazard.tbl row. `HazardSuspend` selector `0` targets the id-modifier array while selector `2` targets the referenced row's hazard type; duration/persistence cleanup reverses enable and suspension contributions. The three proportional-looking `Clean Air` operation `4` rows remain diagnostic-only.

Related movement/transport families remain separate or incomplete: `WarplotTeleport`, `RapidTransport`, `GoMap`, `ReturnMap`, `ChangePlane`, and the remaining forced-movement parity gaps.
