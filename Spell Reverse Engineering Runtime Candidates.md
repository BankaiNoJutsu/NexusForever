...
| Item Visual Swap | Global visual rows | Use query 28 | `DataBits00` raw client visual slot, `DataBits01` display id, `DataBits04/05` colour/dye data; immediate rows should update visuals and duration rows should restore tracked previous slot visuals. | `/spell inspect4 <spell4>` then `/spell cast4 <spell4>` |
