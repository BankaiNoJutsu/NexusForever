...
- Implement public-event kill objective progress for starter-zone creature deaths: normal target-group kills update active player event objectives, and entities loaded from `entity_event` retain event id/phase so event-owned creatures can update event-unit and cluster objective variants without advancing object-id `0` objectives for unrelated map kills.
