...
| **Generic table-driven** (supported objective types) | 3846 | 74.0% |
| **Partial** (partial objective mix or stub path scripts) | 9 | 0.2% |
| **Blocked / missing gameplay** (unsupported objective types) | 0 | 0.0% |
| Distinct quest ids with `ScriptFilterOwnerId` | 85 | 1.6% |

### Quests with objectives only

| Tier | Count | % of quests with objectives |
| --- | ---: | ---: |
| Completable (generic + curated) | 3894 | 99.8% |
| Partial | 9 | 0.2% |
| Blocked | 0 | 0.0% |
...
| Bucket | Quests | Notes |
| --- | ---: | --- |
| `generic_table_driven` | 3841 | All objectives supported generically; no custom script |
...
| `curated_partial_stub_script` | 9 | Has quest script file but only logging / path-tutorial stubs |
| `generic_with_script` | 5 | Supported objectives plus extra script (often follow-up grant only) |
...
```text
154, 426, 870, 990, 1387, 1658, 2979, 2997, 3460, 3479, 3480, 3486, 3487, 3667, 3668, 3670, 3671, 3673, 3741, 3777, 3781, 3783, 3797, 3886, 3963, 4526, 4666, 4667, 4696, 5573, 5575, 5580, 5583, 5584, 5593, 5594, 5595, 5596, 5597, 5604, 5610, 8855, 10510, 10513, 10518, 10519, 10520, 10521, 10522, 10523, 10524, 10525, 10526, 10527, 10528, 10530, 10532, 10540, 10541, 10544, 10545, 10546, 10547, 10548, 10549, 10550, 10551, 10552, 10553, 10554, 10555, 10556, 10557, 10558, 10559, 26443, 37302, 56145, 56146, 56147, 56148, 56149, 56150, 56151, 56152
```
