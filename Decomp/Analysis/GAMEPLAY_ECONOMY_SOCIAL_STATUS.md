...
| F-008 Crafting | Partial | `CraftingLootIdCraftHandlerTests` pins LootId craft path; simple/craft-item tests now pin non-zero station/tradeskill mismatch rejection; `CraftingAdditiveHandlerTests` pins additive zero-station rejection; `CraftingPacketShapeTests` pin corrected typed `0x084B`/`0x0855` auxiliary payloads | Discovery math; exact station service-key meanings/zero-station policy; aux opcode emit intent remains unmapped; rune/sigil semantics |
...
| F-010 Group/matching | Partial | `GroupLootRulesHandlerTests`; `ServerGroupInstanceDifficultyResponse` (`0x0414`) typed + fan-out on `ClientGroupSetInstanceDifficulty`; ready-check status (`0x0441`) now emits for Pending/Ready/HasSetReady flag updates; replacement LFR opens/closes an in-progress anchor queue and accepted proposals merge into the active PvE match | `ServerRaidQueueStatus` (`0x0718`) emit path; `Client0x062A/0634`; durable raid locks; queue vote/multi-slot replacement lifecycle |
...
`dotnet test Source/NexusForever.Game.Tests/NexusForever.Game.Tests.csproj --no-restore -p:UseSharedCompilation=false -m:1 -v minimal --nologo` -> **873 passed** (2026-05-23).
