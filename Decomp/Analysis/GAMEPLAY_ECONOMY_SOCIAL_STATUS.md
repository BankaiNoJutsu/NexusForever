...
| F-008 Crafting | Partial | **New** `CraftingLootIdCraftHandlerTests` pins LootId craft path; existing simple/craft-item tests; `CraftingPacketShapeTests` now pin typed `0x084B`/`0x0855` auxiliary payloads | Discovery/station math; aux opcode emit intent remains unmapped; rune/sigil semantics |
...
| F-010 Group/matching | Partial | `GroupLootRulesHandlerTests`; `ServerGroupInstanceDifficultyResponse` (`0x0414`) typed + fan-out on `ClientGroupSetInstanceDifficulty`; ready-check status (`0x0441`) now emits for Pending/Ready/HasSetReady flag updates | `ServerRaidQueueStatus` (`0x0718`) emit path; `Client0x062A/0634`; durable raid locks; queue vote/replacement lifecycle |
