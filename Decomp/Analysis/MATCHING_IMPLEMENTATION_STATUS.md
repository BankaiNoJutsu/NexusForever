...
Results: test project passed `920/920`; solution build succeeded with `0` warnings and `0` errors.
...
| Replacement queue fill | Client senders are mapped: `MatchingReplacement_SendStartLookingForReplacements` @ `14076aa30` sends `0x05D5` with a 32-bit role bitmask made from selected values `0..2`, and `MatchingReplacement_SendStopLookingForReplacements` @ `14076abb0` sends empty `0x0602`. Handlers validate the sender's current in-progress match, so the target match/team is server-inferred. | Retail accept/fill packet sequence for adding accepted candidates to the existing match/team/role slot. |
